...
class Attention(nn.Module):
    ...
    def forward(self, x, act_scaling_factor):
        ...
        attn, act_scaling_factor = self.matmul_1(
            q, act_scaling_factor_1, k.transpose(-2, -1), act_scaling_factor_1
        )
...
class Block(nn.Module):
    ...
    def forward(self, x_1, act_scaling_factor_1):
        ...
        x = self.drop_path(x)
        x_2, act_scaling_factor_2 = self.qact2(
            x, act_scaling_factor, x_1, act_scaling_factor_1
        )
        ...
        x = self.drop_path(x)
        x, act_scaling_factor = self.qact4(
            x, act_scaling_factor, x_2, act_scaling_factor_2
        )
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x, act_scaling_factor = self.qact1(
            x, act_scaling_factor, x_pos, act_scaling_factor_pos
        )
        x = self.pos_drop(x)
